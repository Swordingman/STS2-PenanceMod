...
public partial class SkinSelectorUI : Control
{
    ...
    public override void _Ready()
    {
        ...
        if (_leftBtn != null)
        {
            _leftBtn.TextureNormal = leftArrowTex;
        }

        if (_rightBtn != null)
        {
            _rightBtn.TextureNormal = rightArrowTex;
        }

        // 5. 绑定左右箭头点击事件
        if (_leftBtn != null)
        {
            _leftBtn.Pressed += OnLeftPressed;
        }

        if (_rightBtn != null)
        {
            _rightBtn.Pressed += OnRightPressed;
        }
        ...
        if (_challengesVBox != null)
        {
            ...
            while (true)
            {
                ...
                if (LocString.Exists("relics", locKey))
                {
                    ...
                    {
                        Text = new LocString("relics", locKey).GetFormattedText(),
                        ButtonPressed = PenanceConfig.EnabledChallenges.Contains(index)
                    };
    ...
    private void PlayIdleAnimation(Node rootNode)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            if (current is NCreatureVisuals visuals)
            {
                try
                {
                    // 1. 单独调用设置动画的方法，不接收返回值
                    visuals.SpineAnimation.SetAnimation("idle_loop");

                    // 2. 然后获取当前播放的动画轨道
                    MegaTrackEntry? entry = visuals.SpineAnimation.GetCurrentTrack(0);
                    ...
                    if (entry != null)
                    {
                        ...
                        {
                            entry.SetTrackTime(
                                (animationEnd + MegaCrit.Sts2.Core.Random.Rng.Chaotic.NextFloat(-0.5f, 0.5f)) % animationEnd
                            );
                        }
                    }
